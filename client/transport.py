...
class ClientTransport(threading.Thread, QObject):
    ...
    def __init__(self, port, ip_address, database, username):
        ...
        try:
            ...
            LOGGER_FOR_CLIENT.error('Timeout соединения при обновлении списков пользователей.')
    ...
    def connection_init(self, port, ip):
        ...
        LOGGER_FOR_CLIENT.debug('Установлено соединение с сервером')
        try:
            with socket_lock:
            ...
            raise ServerError('Потеряно соединение с сервером!')
        LOGGER_FOR_CLIENT.info('Соединение с сервером успешно установлено.')
    ...
    def create_presence(self):
        ...
        LOGGER_FOR_CLIENT.debug(f'Сформировано {PRESENCE} сообщение для пользователя {self.username}')
        return out
    ...
    def process_server_ans(self, message):
        ...
        if RESPONSE in message:
            if message[RESPONSE] == 200:
                ...
                LOGGER_FOR_CLIENT.debug(f'Принят неизвестный код подтверждения {message[RESPONSE]}')
        elif ACTION in message and message[ACTION] == MESSAGE and SENDER in message and DESTINATION in message \
                and MESSAGE_TEXT in message and message[DESTINATION] == self.username:
            LOGGER_FOR_CLIENT.debug(f'Получено сообщение от пользователя {message[SENDER]}:{message[MESSAGE_TEXT]}')
