...
from common.variables import ACTION, EXIT, TIME, ACCOUNT_NAME, MESSAGE, SENDER, DESTINATION, MESSAGE_TEXT, PRESENCE, USER, RESPONSE, ERROR, DEFAULT_IP_ADDRESS, DEFAULT_PORT, ADD_CONTACT, \
    USERS_REQUEST, LIST_INFO, REMOVE_CONTACT, GET_CONTACTS
    ...
    def create_message(self):
        ...
        with database_lock:
            if not self.database.check_user(to):
                LOGGER_FOR_CLIENT.error(f'Попытка отправить сообщение незарегистрированому получателю: {to}')
                return
        ...
        with sock_lock:
            try:
                ...
                if err.errno:
                    ...
                    LOGGER_FOR_CLIENT.error('Не удалось передать сообщение. Таймаут соединения')
        ...
        if ans == 'del':
            ...
            with database_lock:
                if self.database.check_contact(edit):
                    ...
                    LOGGER_FOR_CLIENT.error('Попытка удаления несуществующего контакта.')
        elif ans == 'add':
            ...
            if self.database.check_user(edit):
                ...
                with sock_lock:
                    try:
                        ...
                        LOGGER_FOR_CLIENT.error('Не удалось отправить информацию на сервер.')
                    ...
                    if ACTION in message and message[ACTION] == MESSAGE and SENDER in message and DESTINATION in message \
                        ...
                        with database_lock:
                            try:
                                ...
                                LOGGER_FOR_CLIENT.error('Ошибка взаимодействия с базой данных')
...
def contacts_list_request(sock, name):
    LOGGER_FOR_CLIENT.debug(f'Запрос контакт листа для пользователся {name}')
    req = {
        ACTION: GET_CONTACTS,
        TIME: time.time(),
        USER: name
...
def user_list_request(sock, username):
    LOGGER_FOR_CLIENT.debug(f'Запрос списка известных пользователей {username}')
    req = {
...
def database_load(sock, database, username):
    try:
        ...
        LOGGER_FOR_CLIENT.error('Ошибка запроса списка известных пользователей.')
    else:
    ...
    try:
        ...
        LOGGER_FOR_CLIENT.error('Ошибка запроса списка контактов.')
    else:
        for contact in contacts_list:
